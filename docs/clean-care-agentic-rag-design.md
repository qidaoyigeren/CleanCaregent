...
- 完整 MCP 协议、分布式 Agent 调度、大规模多租户平台。
...
| Tool Executor | 工具注册、Schema 校验、用户权限、超时、幂等、审计和失败映射 |
...
系统借鉴 MCP 的“工具声明 + Schema + 执行器”思想，但不实现 MCP 传输、发现和完整协议。
...
执行链路：白名单检查 -> JSON Schema 校验 -> 用户/订单权限检查 -> 重复/幂等检查 -> `context.WithTimeout` -> Execute -> 脱敏 -> 写 `tool_call_logs` -> 转换为 Evidence。
...
- “参考 MCP Tool Registry 思想设计了 Schema 化工具注册与执行”，不能写“实现完整 MCP 协议”。
...
- **功能**：15 类意图、Query Rewriter、Hybrid Planner、5 个 Skill、6 个 mock 工具、Tool Registry、Evidence Collector、Reflection 和多轮故障状态。
