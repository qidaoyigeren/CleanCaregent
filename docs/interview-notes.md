...
CleanCare Agent 是 Go 实现的清洁电器垂直 Agentic RAG 后端，只覆盖扫地机器人、空气净化器、净水器和加湿器。143 篇 mock 知识文档承载参数、对比、说明书、故障树和政策，300 条 v2 单轮评测覆盖纯 KB、纯 Tool、KB+Tool 与拒答引导，并分为 200 条 regression、75 条 tuning、25 条 holdout。简单问题走快速 RAG，复杂问题走 ReAct 或 Plan-and-Execute，动态价格、库存、订单、保修和工单通过受控工具完成。
...
| 评测闭环 | 300 条数据集同时统计自纠正、安全、有据率和工具利用率 | `internal/eval/metrics.go`、`internal/eval/runner.go` | 不只看答案相似度，还定位 Agent 链路失败位置 |
...
2026-06-13 的真实模型 200 条严格评测从 31.0% 提升到 71.5%：
Intent Accuracy 96.0%，Hit@5 94.5%，Multi-step Completion 88.0%。
对比意图 14/17（82.4%），推荐意图 21/26（80.8%），创建售后工单
7/7。平均 Token 从 9,838 降到 4,007，P95 从 26.8 秒降到 17.1 秒。
这是单机串行、Mock 业务数据下的结果，不等同于生产 SLA。

### 项目哪里仍不是生产完成态

所有业务数据是 mock；没有接入真实支付、物流或 ERP。真实模型全量评测已执行，
但 50/100 并发压测、长期线上告警和生产 SLA 验证尚未完成，文档明确区分
“已实现”和“待验证”。
