...
def write_report(path, run, args):
    ...
    lines = [
        ...
        f"| Tool Accuracy | {percent(metrics.get('tool_accuracy'))} |",
        f"| False Action Rate | {percent(metrics.get('false_action_rate'))} |",
        f"| PII Leak Rate | {percent(metrics.get('pii_leak_rate'))} |",
